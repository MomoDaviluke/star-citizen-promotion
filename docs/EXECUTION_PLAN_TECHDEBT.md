...
| C6 | 测试基线不得回退：后端 **656/656**、前端 **616/616**（2026-08-31 实测；TD-28 后与 Node 版本无关） |
...
npm test                     # vitest run（全量单测，基线 616/616）
...
| M1 | AI-DEP-1/2 部署验证 | P1 | 中 | 需真实 LLM key | ✅ 已完成 2026-08-30（AI-DEP-2 转 M2-3） |
| M2 | 生产部署准备 | P0 | 中 | 需服务器权限 | 📋 排队（**唯一未完成 P0**） |
| M3 | E2E 扩展（fleet/admin） | P2 | 中 | M0 完成（admin 链路联动） | ✅ 已完成 2026-08-30 |
| M4 | 前端覆盖率 → 70% | P2 | 大 | 串行推进 | 🔄 进行中（门禁 60→65，2026-08-31） |
| M5 | AI Phase 2: GEO 优化 | P2 | 3 天 | M1 验证通过后 | ⛔ 已搁置（2026-08 方向转向） |
| M6 | AI Phase 3: 选舰助手 | P2 | 1 周 | M1 | ⛔ 已搁置（2026-08 方向转向） |
| M7 | 远期演进 | P3 | — | 按需 | ⚪ 远期 |

**执行顺序：M0 ✅ → M1 ✅ → M3 ✅ → M4 进行中 → M2（待服务器权限）→ M7 按需；M5/M6 已搁置。**
