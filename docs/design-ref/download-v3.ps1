...
foreach ($section in $sections) {
  ...
  # Use curl to get the redirect URL (don't follow redirects)
  $curlOutput = & curl -s -D - -o NUL $apiUrl 2>&1
  $locationLine = $curlOutput | Select-String -Pattern "^Location:" | Select-Object -First 1

  if ($locationLine) {
    $imageUrl = $locationLine.ToString().Trim() -replace "^Location:\s*", ""
    Write-Host "  Image URL: $imageUrl"

    if ($imageUrl -match 'default\.jpeg$') {
      Write-Host "  WARNING: Got default placeholder image"
    }

    # Download the actual image
    Invoke-WebRequest -Uri $imageUrl -OutFile $outPath -TimeoutSec 120
    $len = (Get-Item $outPath).Length
    Write-Host "  Downloaded: $len bytes"
  } else {
    Write-Host "  FAILED: No Location header found"
    Write-Host "  Response: $curlOutput"
  }
}
