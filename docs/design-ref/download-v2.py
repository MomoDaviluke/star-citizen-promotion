...
# Try with different User-Agent and Referer headers to see if API needs them
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
    'Accept-Language': 'en-US,en;q=0.5',
    'Referer': 'https://www.trae.com/',
    'Origin': 'https://www.trae.com',
}
    ...
    try:
        ...
        try:
            ...
            if 'default.jpeg' in location:
                print(f"  WARNING: Got default placeholder - trying with cookies/session")

                # Try following redirects with full browser headers
                opener_full = urllib.request.build_opener()
                req2 = urllib.request.Request(url, headers=headers)
                try:
                    response2 = opener_full.open(req2, timeout=120)
                    data2 = response2.read()
                    with open(out_path, 'wb') as f:
                        f.write(data2)
                    print(f"  Followed redirect: {len(data2)} bytes")
                except Exception as e2:
                    print(f"  Follow redirect also failed: {e2}")
            else:
...
for filename, _ in sections:
    ...
    if os.path.exists(path):
        ...
        print(f"  {filename}: {size} bytes, JPEG={is_jpeg}, default={is_default}")
    else:
