...
$tasks = @(
  ...
  @{
    ...
    Prompt = "Premium dark sci-fi about page section, left side shows three dossier cards stacked vertically with DOSSIER-01 DOSSIER-02 DOSSIER-03 labels in electric cyan, large watermark numbers in background, right side shows sticky data panel with massive statistics 128 47 36 87 percent in white with cyan accent labels, vertical cyan data-flow line between columns, subtle noise texture, awwwards quality, Star Citizen organization profile, OLED black theme"
  },
  @{
...
foreach ($task in $tasks) {
  $encoded = [Uri]::EscapeDataString($task.Prompt)
  $url = "${base}?prompt=${encoded}&image_size=landscape_16_9"
  $outPath = Join-Path $outDir $task.File
  Write-Host "Downloading $($task.File)..."
  try {
...
Write-Host "`nAll done. Listing output:"
Get-ChildItem $outDir -Filter "section-*.png" | ForEach-Object { Write-Host "  $($_.Name) - $($_.Length) bytes" }
