...
│   │   ├── useAI.js               # AI服务封装
│   │   ├── useParallax.js         # 视差效果
│   │   ├── useScrollReveal.js     # 滚动展示
│   │   ├── useSoundEffect.js      # 音效
...
│   ├── join.spec.js               # 申请页测试
│   ├── auth.spec.js               # 认证流程测试
│   ├── apply.spec.js              # 申请流程测试
│   └── navigation.spec.js         # 导航与响应式测试
│
└── docs/                          # 文档目录
    ├── API.md                     # API文档
    ├── CONFIG.md                  # 配置文档
...
> **⚠️ 后端测试必须用 `npm test`（在 server/ 目录），不要直接 `npx jest`**。
> 项目脚本为 `node --import tsx --experimental-vm-modules jest.js`，依赖 tsx loader 处理 ESM/TS。直接 `npx jest` 会因 `__filename` 与 CJS 包装冲突导致大量 suite 加载失败（2026-08-05 实测 30/46 suites 报 SyntaxError），这是命令调用方式问题，不是代码问题。
>
> ```bash
> cd server && npm test    # ✅ 正确（46 suites / 469 tests）
> npx jest                 # ❌ 错误（缺 tsx loader，加载失败）
> ```
...
// 使用 Vue Devtools 浏览器扩展查看组件状态
```

#### 网络请求调试

```javascript
// 使用浏览器 Network 面板查看请求，或在开发环境启用 requestLogger 中间件
// 生产环境使用 Winston 结构化日志，详见 server/src/middleware/requestLogger.ts
```
...
logger.info(`${req.method} ${req.url}`, { requestId: req.id, duration })
```

#### 数据库查询调试

```typescript
// server/src/database/pool.ts — 使用 queryWithTiming 自动记录慢查询
import { queryWithTiming } from '../database/pool'
// 超过 500ms 的查询会自动告警
const rows = await queryWithTiming('SELECT * FROM users WHERE id = ?', [userId])
```
