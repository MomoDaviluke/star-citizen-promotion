...
| 项 | 原因 |
|:---|:---|
| TypeScript 迁移 | 工作量大，建议独立迭代 |
| i18n 国际化 | 需产品设计配合，建议独立迭代 |
| Pinia 状态管理 | 涉及全前端重构，建议独立迭代 |
| 数据库迁移框架 | 需选型评估（knex/Sequelize），建议独立迭代 |
| 集中式表单验证 | 需选型（VeeValidate），建议独立迭代 |
| httpOnly cookie JWT | 需前后端协同改造，建议独立迭代 |
| 软删除机制 | 涉及全表改造，建议独立迭代 |
