...
> **更新日期**: 2026-08-27
> **版本**: v1.6.2
> **架构风格**: 分层架构 + 模块化前端 + AI 服务层（LLM/RAG/SSE）
...
│   ├── statsService.js # 统计数据
│   ├── rumService.js   # 真实用户性能监控（RUM）
│   └── planet/         # 行星渲染（ProceduralMarsRenderer / TexturedSphereRenderer 等）
├── stores/             # Pinia 状态
│   ├── auth.js         # 认证状态
│   ├── calendar.js     # 日历状态
│   ├── fleet.js        # 舰队状态
│   └── homeStore.js    # 首页状态
├── composables/        # 组合式函数
│   ├── useAiRecruiter.js # AI 招募官（SSE 流式对话 + 画像预填）
│   ├── useGSAPReveal.js  # GSAP 滚动动画
│   ├── useMotion.js      # 动效分级（替代旧 useEffectQuality）
│   ├── usePwa.js         # PWA 生命周期
│   ├── useTheme.js       # 主题切换
│   └── useWebVitals.js   # 性能指标上报
├── router/             # 路由配置
│   └── index.js
├── config/             # 配置文件
│   ├── index.js        # 应用配置聚合
...
**CSS 变量完整性**：`scripts/css-var-lint.mjs` 集成 CI，双层检查（broken→error / deprecated→warning）。当前 171 个定义、47 个别名、109 处 deprecated 引用待迁移（2026-08-26 实测，迁移计划见 [TODO.md TD-13](TODO.md)）。

**设计方向**：SpaceX 极简风格。Hero 全屏沉浸、一屏一焦点、留白为武器。
...
> 设计决策见 AD-07 ~ AD-12。采用「基础设施先行」方案：一次投入共享 RAG 基础设施，多消费端（招募官 / GEO / 智能推荐）复用。
...
┌─────────────────────────────────────────────────────────┐
│                      前端（Vue 3）                        │
│  useAiRecruiter（fetch + ReadableStream 解析 SSE）        │
│  components/ai/*（RecruiterTerminal / HoloAvatar /       │
│    ChatStream / QuickSuggestions / ProfilePanel）         │
└──────────────────────┬──────────────────────────────────┘
                       │ POST /api/v1/ai/*
┌──────────────────────▼──────────────────────────────────┐
│                 路由层 routes/ai.ts                       │
│  /health /retrieve /recruiter/session /recruiter/chat    │
│  /recruiter/suggest（inline 限流 10 次/分钟/IP）           │
└──────────────────────┬──────────────────────────────────┘
┌──────────────────────▼──────────────────────────────────┐
│                服务层 services/ai/*                      │
│  LlmService（Redis 缓存 24h）  RagService（检索→组装→调用）│
│  RecruiterService（会话+RAG+画像+SSE 全链路）             │
│  SessionStore（Redis 会话 24h TTL）ProfileEngine（画像）  │
│  providers/（OpenAI 兼容 / Anthropic 工厂 + 降级链）       │
│  embedder / retriever / ingester / promptBuilder          │
└──────────────┬────────────────────────┬─────────────────┘
               ▼                        ▼
     ┌─────────────────┐      ┌─────────────────┐
     │ PostgreSQL      │      │ Redis           │
     │ pgvector 语义检索│      │ LLM 缓存 + 会话  │
     │ knowledge_chunks│      │ (独立于 MySQL)   │
     └─────────────────┘      └─────────────────┘
```
...
| 组件 | 职责 | 说明 |
|:---|:---|:---|
| LLMProvider 抽象 | 统一 `chat/chatStream/embed` 接口 | `providers/types.ts`，适配豆包/DeepSeek（OpenAI 兼容）+ Claude（Anthropic） |
| 降级链 | Provider 路由与失败切换 | `routeWithFallback`，不可重试错误立即抛出不切换；降级深度限 2 次（SEC-05） |
| RAG 引擎 | 知识库语义检索 | Embedder（向量生成+降级）/ Retriever（pgvector top-k + metadata 过滤）/ Ingester（chunk 512/overlap 50，幂等先删后插）/ PromptBuilder（`$` 转义） |
| LlmService | LLM 统一入口 | Redis 缓存 24h，cacheKey 含 temperature/maxTokens（LLM-02） |
| RagService | 检索→组装→调用全链路 | 知识库问答的统一消费端 |
| RecruiterService | 招募官 Agent | 会话 + RAG 检索 + 画像更新 + SSE 流式 |
| SessionStore | 会话存储 | Redis，24h TTL，最近 6 轮历史窗口，JSON.parse 防护 |
| ProfileEngine | 画像引擎 | 关键词规则（playStyle/timeCommit/shipPref/skillLevel），合并而非覆盖 |

### SSE 交互协议

招募官对话使用 SSE（Server-Sent Events）流式返回。**必须使用 `fetch + ReadableStream` 手动解析**（AD-11），因为需要 POST body，EventSource 仅支持 GET。

```
前端 → POST /api/v1/ai/recruiter/chat { sessionId, message }
后端 → SSE 事件流：
      event: delta   data: { text: "..." }        ← 逐字增量
      event: profile data: { key, value }         ← 画像更新
      event: done    data: { sessionId, suggestions }
```

前端画像预填链路：RecruiterTerminal 收集画像 → JSON 编码至 `?ai_profile=`（AD-12）→ Join.vue 解析并预填申请表。

### AI 数据基础设施

| 项 | 说明 |
|:---|:---|
| PostgreSQL 16 (pgvector) | 独立于 MySQL 业务库（AD-08），`knowledge_chunks` 表 + ivfflat 余弦索引 |
| Redis | 双用途：LLM 响应缓存 + AI 会话存储 |
| docker-compose | 新增 `postgres` / `redis` 服务 |
| 脚本 | `ai:ingest`（知识入库）/ `ai:migrate`（向量库迁移） |
...
| 并发用户 | 1000 | 未测试 | ⚠️ 待验证 |
| 测试覆盖率 (后端) | ≥ 60% | 75.35%（AI 模块 88.94%） | ✅ 已达标 |
| 测试覆盖率 (前端) | ≥ 80% | ~60% | 📋 待提升 |
