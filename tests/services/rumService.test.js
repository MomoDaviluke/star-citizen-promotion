...
describe('rumService', () => {
  ...
  it('存在 sendBeacon 时应优先使用 sendBeacon', async () => {
    ...
    // sendBeacon 直接传字符串会被浏览器强制置为 text/plain，
    // 后端 express.json() 不解析该类型导致 req.body 为空而被白名单校验拒绝（400）。
    // 因此必须包装为 application/json 的 Blob。
    expect(sendBeacon).toHaveBeenCalledWith('/api/v1/rum', expect.any(Blob))
  ...
  it('sendBeacon 返回 false 时应降级到 fetch keepalive', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/v1/rum', {
      method: 'POST',
  ...
  it('无 sendBeacon 时应降级到 fetch keepalive', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledTimes(1)
    expect(global.fetch).toHaveBeenCalledWith('/api/v1/rum', expect.objectContaining({
      method: 'POST',
      keepalive: true
    }))
  })
