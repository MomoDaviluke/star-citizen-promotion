...
describe('HTTP 客户端', () => {
  ...
  describe('HTTP 方法', () => {
    ...
    it('GET 应正确拼接查询参数', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/v1/test?page=1&limit=10', expect.objectContaining({ method: 'GET' }))
    })

    it('GET 应过滤 null 和 undefined 参数', async () => {
      await httpClient.get('/test', { page: 1, limit: null, offset: undefined })
      expect(mockFetch.mock.calls[0][0]).toBe('/api/v1/test?page=1')
    })
    ...
    it('DELETE 应发送无请求体', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/v1/test/1', expect.objectContaining({ method: 'DELETE' }))
    })
  })
