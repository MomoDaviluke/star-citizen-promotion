...
function openCreateModal() {
  ...
  editingPilot.value = {}
  editForm.value = {
    name: '', callsign: '', ship: '', description: '',
    missions: 0, kills: 0, status: 'active'
  }
}
...
async function saveEdit() {
  ...
  try {
    const response =
      modalMode.value === 'edit'
        ? await dataService.updatePilot(editingPilot.value.id, editForm.value)
        : await dataService.createPilot(editForm.value)
    if (response.success) {
