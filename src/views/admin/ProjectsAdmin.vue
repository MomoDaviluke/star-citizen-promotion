...
function openCreateModal() {
  ...
  editForm.value = { name: '', period: '', description: '', status: 'active', progress: 0 }
}
...
async function saveEdit() {
  ...
  try {
    const response =
      modalMode.value === 'edit'
        ? await dataService.updateProject(editingProject.value.id, editForm.value)
        : await dataService.createProject(editForm.value)
    if (response.success) {
