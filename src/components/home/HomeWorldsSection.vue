...
import { initParallax } from '../../composables/useGSAPReveal.js'
...
let ctx = null
// 星云视差的 ScrollTrigger（initParallax 自建实例，不纳入 ctx，需手动清理）
let nebulaParallax = null
...
onMounted(() => {
  ...
  // 星云背景微视差：随滚动正向缓移，与前景行星形成纵深
  const nebulaEl = section.querySelector('.worlds-section__nebula')
  if (nebulaEl) {
    nebulaParallax = initParallax(nebulaEl, {
      distance: 40,
      trigger: section,
      start: 'top bottom',
      end: 'bottom top',
...
.worlds-section__planet-wrap {
  ...
  aspect-ratio: 1 / 1;
  flex-shrink: 0;
  /* 仅 layout：paint 裁剪会把超出 wrap 的轨道环切掉，环由 section overflow:hidden 兜底 */
  contain: layout;
}

.worlds-section__planet {
  width: 100%;
  /* height:auto：让组件内联 aspect-ratio 生效，避免 wrap 高度把行星拉成椭圆 */
  height: auto;
...
/* 窄屏阅读层级收缩：标题随视口降档，避免 40px 大标题挤压正文 */
