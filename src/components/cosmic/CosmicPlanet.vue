...
const isTextureVisible = ref(false)
...
/**
 * 当纹理行星进入视口时，异步加载纹理并标记就绪
 */
function initTextureLazyLoad() {
  if (!props.texture || !planetRef.value || typeof IntersectionObserver === 'undefined') {
    // 无纹理或环境不支持时直接就绪，保证非懒加载场景正常显示
  ...
  textureObserver = new IntersectionObserver(
    async (entries) => {
      ...
      if (visible && !isTextureVisible.value) {
        isTextureVisible.value = true
        try {
          await preloadTexture(props.texture)
          isTextureReady.value = true
        } catch (err) {
          // 预加载失败时保持未就绪状态，组件会自动回退到默认渐变
          // eslint-disable-next-line no-console
          console.warn('[CosmicPlanet] texture preload failed:', props.texture, err)
        }
  ...
  textureObserver.observe(planetRef.value)
}
...
function initTexturedSphere(canvas) {
  ...
  img.onload = () => {
    ...
    // 1/3 分辨率渲染：像素数减少 9 倍，再由 Canvas 平滑放大到显示尺寸
    ...
    /**
     * 简化版渲染：漫反射光照 + 极地冰冠增强 + 边缘消隐
     * 限制为 30fps，降低 CPU/GPU 占用
     */
    const render = (time) => {
      if (!isActive) return

      // 限制火星渲染帧率为 30fps
      if (time - lastFrameTime < 33.33) {
      ...
      if (isVisible) {
        rotation += (2 * Math.PI) / (props.rotationDuration * 30)
...
function initProceduralMars(canvas) {
  ...
  const render = (time) => {
    ...
    // 限制为 30fps，并在离屏时跳过渲染
    if (time - lastFrameTime < 33.33 || !isVisible) {
      rafId = requestAnimationFrame(render)
      return
    }
    lastFrameTime = time
    ...
    const speed = (surfaceWidth / (props.rotationDuration * 60)) * 2
...
onMounted(() => {
  ...
  // 离屏时暂停 Canvas 动画，减少非可视区域的 CPU 开销
  ...
  if (marsCanvas.value) {
    observer.observe(marsCanvas.value)
  }
})
