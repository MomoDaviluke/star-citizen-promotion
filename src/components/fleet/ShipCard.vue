...
<template>
  ...
  >
    <div class="ship-card__shell">
      <div class="ship-card__glow" aria-hidden="true"></div>
...
// ── hover 3D tilt + 辉光跟随（仅 ≥769px 桌面；transform/opacity-only） ──
const cardRef = ref(null)
let pointerCleanup = null

onMounted(() => {
  const card = cardRef.value
  const shell = card?.querySelector('.ship-card__shell')
  if (!card || !shell) return
  // 降级：减少动画偏好 / 触摸设备（无 hover 语义）不启用 tilt
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return
  if (!window.matchMedia('(min-width: 769px)').matches) return

  // quickTo 内建 rAF 节流；rotation 上限 ±6deg
  const rotX = gsap.quickTo(shell, 'rotationX', { duration: 0.5, ease: 'power2.out' })
  const rotY = gsap.quickTo(shell, 'rotationY', { duration: 0.5, ease: 'power2.out' })
  const liftY = gsap.quickTo(shell, 'y', { duration: 0.4, ease: 'power2.out' })

  const onEnter = () => liftY(-4) // hover 上浮（原 CSS translateY 由 GSAP 接管）
  const onMove = (e) => {
    const r = card.getBoundingClientRect()
    const px = (e.clientX - r.left) / r.width // 0~1 指针相对位置
    const py = (e.clientY - r.top) / r.height
    rotY((px - 0.5) * 12)
    rotX((0.5 - py) * 12)
    // 辉光跟随：CSS 变量驱动 glow 层的 radial-gradient 圆心
...
.ship-card {
  cursor: pointer;
  perspective: 900px; /* 3D 透视基座：子元素 shell 的 rotationX/Y 由此呈现纵深 */
}

.ship-card__shell {
  position: relative; /* glow 辉光层的定位上下文 */
  background: var(--color-bg-glass);
  border: 1px solid var(--color-border-hover);
  border-radius: var(--radius-2xl);
  padding: 6px;
  /* transform（rotationX/Y + hover 上浮 y）由 GSAP quickTo 接管，
   * 不写 CSS transform transition 避免 inline style 与 transition 抢动 */
  transition:
...
/* 辉光跟随层：radial-gradient 圆心由 --mx/--my（JS pointermove 驱动）定位 */
.ship-card__glow {
  position: absolute;
  inset: 0;
  z-index: 5;
...
.ship-card:hover .ship-card__glow {
  opacity: 1;
}
