...
// bcrypt.compare 用于 confirmPassword 验证
...
// queryOne 用于 verifyAdminPassword 查 password_hash
...
/**
 * 设置密码验证通过的 mock 链
 * queryOne 返回含 password_hash 的用户记录 → bcrypt.compare 返回 true
 */
function setupPasswordVerification() {
  mockQueryOne.mockResolvedValue({ password_hash: 'hashed-password' })
