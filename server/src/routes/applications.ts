...
router.get(
  '/:id',
  validate([param('id').notEmpty().withMessage('申请 ID 不能为空')]),
  optionalAuth,
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      const application = await getApplicationById(req.params.id as string)
      if (!application) throw ApiError.notFound('申请不存在')

      if (req.user?.role !== 'admin' && application.email !== req.user?.email) {
        if (!req.user) throw ApiError.unauthorized('请先登录后查看申请')
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(201).json({
        ...
        data: { id: newApplication.id, status: newApplication.status, createdAt: newApplication.createdAt }
      })
    } catch (error) {
      next(error)
    }
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const updatedApplication = await updateApplicationStatus(req.params.id as string, status, req.user!.id, note)
      res.json({ success: true, message: '申请状态更新成功', data: updatedApplication })
    } catch (error) {
      next(error)
    }
  }
