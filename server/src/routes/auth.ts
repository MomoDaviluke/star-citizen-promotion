...
router.post('/login', validate(loginValidation), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put(
  ...
  authenticate,
  validate([
    body('username').optional().trim().isLength({ min: 3, max: 20 }).withMessage('用户名长度需在 3-20 个字符之间'),
...
router.put(
  ...
  validate([
    ...
    body('newPassword')
      .isLength({ min: 8 }).withMessage('新密码长度至少 8 个字符')
...
router.post('/refresh', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!token) throw ApiError.unauthorized('缺少认证令牌')

    const result = await refreshUserToken(token)
    res.cookie('auth_token', result.token, COOKIE_OPTIONS)
    res.json({ success: true, message: '令牌刷新成功', data: result })
  } catch (error) {
    next(error)
  }
})
