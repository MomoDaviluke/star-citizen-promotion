...
router.get(
  '/:id',
  validate([param('id').notEmpty().withMessage('成员 ID 不能为空')]),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const member = await getMemberById(req.params.id as string)
      if (!member) throw ApiError.notFound('成员不存在')
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(201).json({ success: true, message: '成员创建成功', data: newMember })
    } catch (error) {
      next(error)
    }
  }
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({ success: true, message: '成员信息更新成功', data: updatedMember })
    } catch (error) {
      next(error)
    }
  }
