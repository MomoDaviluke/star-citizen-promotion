...
export async function updateEvent(id: string, data: Partial<CalendarEvent>): Promise<CalendarEvent> {
  return transaction<CalendarEvent>(async (conn: PoolConnection) => {
    ...
    const [result] = await conn.execute<ResultSetHeader>('UPDATE events SET ' + setClause + ' WHERE id = ?', values as never)
...
/**
 * 转义 ICS 文本字段
 * @description RFC 5545 规定反斜杠、分号、逗号需转义（前缀 `\`），
 *              换行符需转为 `\n` 字面量。未转义会导致日历解析器误读字段结构。
 * @param {string} value - 原始文本
 * @returns {string} 转义后的文本
 */
function escapeICS(value: string): string {
  return value
    .replace(/\\/g, '\\\\')   // 反斜杠优先（否则会破坏后续转义）
    .replace(/;/g, '\\;')     // 分号（属性参数分隔符）
    .replace(/,/g, '\\,')     // 逗号（值列表分隔符）
    .replace(/\n/g, '\\n')    // 换行 → 字面量 \n
}
