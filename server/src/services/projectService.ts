...
export async function updateProject(id: string, data: Partial<Project>): Promise<Project> {
  return transaction<Project>(async (conn: PoolConnection) => {
    ...
    const [result] = await conn.execute<ResultSetHeader>('UPDATE projects SET ' + setClause + ' WHERE id = ?', [...values, id] as never)
    if (result.affectedRows === 0) throw ApiError.notFound('项目不存在')
