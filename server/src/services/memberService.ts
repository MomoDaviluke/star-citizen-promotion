...
export async function updateMember(id: string, data: Partial<Member>): Promise<Member> {
  return transaction<Member>(async (conn: PoolConnection) => {
    ...
    const [result] = await conn.execute<ResultSetHeader>('UPDATE members SET ' + setClause + ' WHERE id = ?', [...values, id] as never)
    if (result.affectedRows === 0) throw ApiError.notFound('成员不存在')
